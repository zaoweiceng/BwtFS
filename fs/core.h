...
class BwtFS {
    ...
    public:
        BwtFS(std::string system_file_path, std::string mount_dir_path, std::string initial_dir_path):
            system_manager_(system_file_path, mount_dir_path),
            file_manager_(initial_dir_path){}
        ~BwtFS(){
        }
        int open(const std::string& path);
        int read(int fd, char* buf, size_t size);
