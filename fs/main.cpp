...
#elif defined(__APPLE__)
    // macOS使用标准mode_t类型
static int myfs_create_fuse(const char* path, mode_t mode, struct fuse_file_info *fi) {
    (void)mode; // 避免未使用参数警告，目前我们的简单实现忽略mode参数
#else
    // Linux使用标准mode_t类型
static int myfs_create_fuse(const char* path, mode_t mode, struct fuse_file_info *fi) {
    ...
    // 首先尝试打开文件，如果文件不存在则创建
    int fd = myfs.open(path);  // open函数内部会自动创建不存在的文件
    if (fd < 0) {
        // 如果打开失败，尝试显式创建文件
        if (myfs.create(path) < 0) {
...
static int myfs_chmod_fuse(const char *path, fuse_mode_t mode, struct fuse_file_info *fi) {
    ...
    // Windows的chown操作
static int myfs_chown_fuse(const char *path, fuse_uid_t uid, fuse_gid_t gid, struct fuse_file_info *fi) {
...
static int myfs_chown_fuse(const char *path, uid_t uid, gid_t gid, struct fuse_file_info *fi) {
    // 简化实现：忽略所有者修改，总是返回成功
    (void)path; (void)uid; (void)gid; (void)fi;
    return 0;
}
...
static int myfs_chown_macos_chown_adapter(const char *path, uid_t uid, gid_t gid) {
    // 调用我们的实现，忽略第四个参数
...
int main(int argc, char *argv[]){
...
#ifdef _WIN32
    // Windows WinFSP API
    myfs_oper.getattr = myfs_getattr;    // 获取文件属性
    myfs_oper.readdir = myfs_readdir;    // 读取目录内容
    myfs_oper.open    = myfs_open_fuse;  // 打开文件
    ...
    myfs_oper.getattr = myfs_getattr_macos_adapter;     // getattr函数需要适配
    myfs_oper.readdir = myfs_readdir;     // readdir函数签名已适配
    myfs_oper.open    = myfs_open_fuse;   // 打开文件
    myfs_oper.read    = myfs_read_fuse;   // 读取文件内容
    myfs_oper.write   = myfs_write_fuse;  // 写入文件内容
    myfs_oper.unlink  = myfs_unlink_fuse; // 删除文件
