...
#ifdef _WIN32
    // Windows使用fuse_off_t类型
static int bwtfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
                        fuse_off_t offset, struct fuse_file_info *fi,
                        enum fuse_readdir_flags flags)
#elif defined(__APPLE__)
    // macOS使用FUSE 2.9 API，签名略有不同
...
#ifdef _WIN32
    // Windows使用fuse_mode_t类型
static int bwtfs_create_fuse(const char* path, fuse_mode_t mode, struct fuse_file_info *fi) {
    (void)mode; // 避免未使用参数警告，目前我们的简单实现忽略mode参数
#elif defined(__APPLE__)
    // macOS使用标准mode_t类型
...
#ifdef _WIN32
    // Windows使用fuse_off_t类型
static int bwtfs_read_fuse(const char *path, char *buf, size_t size, fuse_off_t offset,
                          struct fuse_file_info *fi)
#elif defined(__APPLE__)
    // macOS使用标准off_t类型
static int bwtfs_read_fuse(const char *path, char *buf, size_t size, off_t offset,
                          struct fuse_file_info *fi)
#else
    // Linux使用标准off_t类型
static int bwtfs_read_fuse(const char *path, char *buf, size_t size, off_t offset,
...
#ifdef _WIN32
    // Windows使用fuse_off_t类型
static int bwtfs_write_fuse(const char *path, const char *buf, size_t size, fuse_off_t offset,
                           struct fuse_file_info *fi)
#elif defined(__APPLE__)
    // macOS使用标准off_t类型
static int bwtfs_write_fuse(const char *path, const char *buf, size_t size, off_t offset,
                           struct fuse_file_info *fi)
#else
    // Linux使用标准off_t类型
static int bwtfs_write_fuse(const char *path, const char *buf, size_t size, off_t offset,
...
// 删除文件 - 跨平台统一的实现
static int bwtfs_unlink_fuse(const char *path){
    return bwtfs.remove(path);
}

// 关闭文件 - 跨平台统一的实现
static int bwtfs_release_fuse(const char *path, struct fuse_file_info *fi){
    return bwtfs.close(fi->fh);
}

// 获取文件系统统计信息 - 跨平台处理不同平台的结构体差异
#ifdef _WIN32
    // Windows WinFSP使用自定义的fuse_statvfs结构体
static int bwtfs_statfs(const char *path, struct fuse_statvfs *stbuf)
#elif defined(__APPLE__)
    // macOS macFUSE使用标准的struct statvfs
...
#ifdef _WIN32
    // Windows使用带flags参数的rename函数
static int bwtfs_rename_fuse(const char *old_path, const char *new_path, unsigned int flags) {
    (void)flags; // flags参数暂时忽略，用于未来的原子性重命名
...
#ifdef _WIN32
    // Windows使用fuse_mode_t类型
static int bwtfs_mkdir_fuse(const char *path, fuse_mode_t mode) {
    (void)mode; // 忽略权限参数
    return bwtfs.mkdir(path);
}
// 文件权限修改 - 跨平台处理不同平台的参数类型差异
    // Windows使用fuse_mode_t类型，但通常不支持chmod操作
static int bwtfs_chmod_fuse(const char *path, fuse_mode_t mode, struct fuse_file_info *fi) {
    // Windows文件系统不支持Unix风格的权限，直接返回成功
    (void)path; (void)mode; (void)fi;
    return 0;
}
    ...
    // Windows的chown操作
static int bwtfs_chown_fuse(const char *path, fuse_uid_t uid, fuse_gid_t gid, struct fuse_file_info *fi) {
    // Windows不支持Unix风格的文件所有权，直接返回成功
    (void)path; (void)uid; (void)gid; (void)fi;
    return 0;
}
...
#ifdef __APPLE__
    // macOS适配函数 - getattr函数没有fuse_file_info参数
static int bwtfs_getattr_macos_adapter(const char *path, struct stat *stbuf) {
    return bwtfs_getattr(path, stbuf, nullptr);
}

    // macOS适配函数 - chmod函数没有fuse_file_info参数
static int bwtfs_chmod_macos_adapter(const char *path, mode_t mode) {
    return bwtfs_chmod_fuse(path, mode, nullptr);
}

    // macOS适配函数 - chown函数没有fuse_file_info参数
static int bwtfs_chown_macos_chown_adapter(const char *path, uid_t uid, gid_t gid) {
    return bwtfs_chown_fuse(path, uid, gid, nullptr);
}
#endif

static struct fuse_operations bwtfs_oper = {};

int main(int argc, char *argv[]){
    // 参数1: 程序名
    // 参数2: 挂载点
    // 若只有两个参数，则使用memory_fs
    // 参数3: Bwtfs系统文件路径
    // 参数4: Bwtfs文件目录的文件路径
    // 初始化FUSE操作结构体
    ...
    LOG_INFO << "Starting FileSystem...";

    // 检查是否提供了挂载点参数
    if (argc < 2) {
        LOG_ERROR << "Usage: " << argv[0] << " <mount_point>";
        return 1;
    }
    if (argc == 2) {
        ...
        try{
            ...
            LOG_ERROR << "Failed to initialize BwtFS: " << e.what();
            return 1;
        }
        #ifdef _WIN32
            // Windows WinFSP API
            ...
            bwtfs_oper.write   = bwtfs_write_fuse; // 写入文件内容
            bwtfs_oper.unlink  = bwtfs_unlink_fuse; // 删除文件
            bwtfs_oper.rename  = bwtfs_rename_fuse; // 重命名/移动文件
            bwtfs_oper.release = bwtfs_release_fuse; // 关闭文件
            bwtfs_oper.statfs  = bwtfs_statfs;     // 获取文件系统统计信息
            bwtfs_oper.create  = bwtfs_create_fuse; // 创建文件
            bwtfs_oper.mkdir   = bwtfs_mkdir_fuse;  // 创建目录
            bwtfs_oper.chmod   = bwtfs_chmod_fuse;  // 修改文件权限
            bwtfs_oper.chown   = bwtfs_chown_fuse;  // 修改文件所有者
            bwtfs_oper.access  = bwtfs_access_fuse; // 检查文件访问权限
        #elif defined(__APPLE__)
            // macOS FUSE 2.9 API - 需要适配不同的函数签名
            bwtfs_oper.getattr = bwtfs_getattr_macos_adapter;     // getattr函数需要适配
            bwtfs_oper.readdir = bwtfs_readdir;     // readdir函数签名已适配
            bwtfs_oper.open    = bwtfs_open_fuse;   // 打开文件
            bwtfs_oper.read    = bwtfs_read_fuse;   // 读取文件内容
            bwtfs_oper.write   = bwtfs_write_fuse;  // 写入文件内容
            bwtfs_oper.unlink  = bwtfs_unlink_fuse; // 删除文件
    ...
    LOG_INFO<< "Starting on Windows with WinFSP...";
    if (argc == 2) {
        return fuse_main_real(my_argc, my_argv, &memory_fs_oper, sizeof(memory_fs_oper), NULL);
    } else {
        return fuse_main_real(my_argc, my_argv, &bwtfs_oper, sizeof(bwtfs_oper), NULL);
    }
#elif defined(__APPLE__)
    // macOS使用标准fuse_main函数
    LOG_INFO << "Starting on macOS with macFUSE...";
    if (argc == 2) {
        return fuse_main(my_argc, my_argv, &memory_fs_oper, NULL);
    } else {
        return fuse_main(my_argc, my_argv, &bwtfs_oper, NULL);
    }
#else
    // Linux使用标准fuse_main函数
    LOG_INFO << "Starting on Linux with libfuse3...";
    if (argc == 2) {
        return fuse_main(my_argc, my_argv, &memory_fs_oper, NULL);
    } else {
        return fuse_main(my_argc, my_argv, &bwtfs_oper, NULL);
